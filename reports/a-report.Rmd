---
title: "Lab. 01 - Parte 2 - AD1/CDD"
author: Francisco Edeverton de Almeida Júnior
output:
    html_document:
    df_print: paged
theme: sandstone
---

```{r echo=FALSE, message=FALSE, warning=FALSE}
library(tidyverse)
library(plotly)
source(here::here("code/lib.R"))
theme_set(theme_bw())
```

```{r read}
dados_parlamentares = read_projectdata()
```

# Introdução
Faremos uma análise exploratória nos dados coletados dos perfis do Twitter dos parlamentares. Acredito que as perguntas que escolhi para responder talvez possam nos surpreender. Decidi deixar abaixo da pergunta o que penso (de acordo com minha opinião sobre o assunto) que podemos obter nas respostas de fato.

# Perguntas
## 1. Qual a quantidade de tweets por partido?
Pensamento inicial: Espero que parlamentares de partidos/estados maiores (geograficamente ou demograficamente) tenham um perfil mais ativo (i.e. publiquem mais conteúdo - próprio ou retweets). Não sei o que esperar quando este agrupamento for feito por casa (câmara/senado).

Levando em consideração todo o conteúdo da rede do parlamentar, dessa forma temos que n_total = n_proprio + n_retweet.
```{r}
dados_parlamentares$n_total <- dados_parlamentares$n_proprio + dados_parlamentares$n_retweet
```

Fazendo o agrupamento por partido e ordenando os dados de maneira decrescente, temos:
```{r}
dados_partidos = dados_parlamentares %>% group_by(partido) %>% summarise(tweets = sum(n_total))
dados_partidos = arrange(dados_partidos, desc(tweets))
```

Agora, tratando um pouco os dados (removendo colunas NA em partido ou tweets):
```{r}
dados_partidos_filtrado = subset(dados_partidos, !is.na(partido) & !is.na(tweets))

dados_partidos_filtrado$partido <- factor(dados_partidos_filtrado$partido, levels = unique(dados_partidos_filtrado$partido)[order(dados_partidos_filtrado$tweets, decreasing = TRUE)])

grafico_partidos_x_tweets <- dados_partidos_filtrado %>% 
  plot_ly(x = ~partido, y = ~tweets) %>%
  layout(title = "Gráfico 1 - Histograma Partido x Núm. tweets", xaxis = list(title = "Partido"), yaxis = list(title = "Núm. tweets")) %>%
  add_bars()

grafico_partidos_x_tweets
```
Conseguimos observar que a quantidade de tweets por partido segue basicamente um padrão onde os maiores partidos do nosso país (como PT, PODE, PSOL) possuem uma quantidade de tweets bem superior a partidos menores como REDE, PATRI, PV, DC, PPL, conforme esperado.

### 1.0. Qual proporção desses tweets foi de fato criada? Qual proporção foi comentada/compartilhada?

### 1.1. Será que a quantidade de parlamentares dos partidos tem uma grande influência nisso? (i.e. será que o PT, possui maior quantidade de tweets pois existem mais parlamentares no partido? Ou seus poucos parlamentares seriam mais ativos em suas redes?)

### 1.2. Os partidos com mais tweets são aqueles que possuem parlamentares mais populares (comparar pelo engajamento)?

## 2. Existe alguma relação entre os engajamentos (total/mediano/máximo) e a quantidade de tweets dos parlamentares?
Pensamento inicial: Dado que existe mais conteúdo para que seus seguidores possam compartilhar, espero que os partidos que tenham parlamentares com perfis mais ativos possuam maiores engajamentos.

Vamos considerar apenas parlamentares que possuam um perfil e tenham postado algo (de autoria própria ou comentando/retweetando). Sendo assim, vamos filtrar o dataset:
```{r}
dados_parlamentares_filtrado = subset(dados_parlamentares, !is.na(twitter) & n_total > 0)
```

Analisando a relação entre o engajamento total dos parlamentares e o número de tweets + retweets:
```{r}
grafico_engaj_total_x_tweets <- dados_parlamentares_filtrado %>%
  plot_ly(
    x = ~n_total,
    y = ~engaj_total
  )

grafico_engaj_total_x_tweets
```

Analisando a relação entre o engajamento mediano dos parlamentares e o número de tweets + retweets:
```{r}
grafico_engaj_mediano_x_tweets <- dados_parlamentares_filtrado %>%
  plot_ly(
    x = ~n_total,
    y = ~engaj_mediano
  )

grafico_engaj_mediano_x_tweets
```

Analisando a relação entre o engajamento máximo dos parlamentares e o número de tweets + retweets:
```{r}
grafico_engaj_max_x_tweets <- dados_parlamentares_filtrado %>%
  plot_ly(
    x = ~n_total,
    y = ~engaj_max
  )

grafico_engaj_max_x_tweets
```


### 2.1. Analisando a mesma situação com o agrupamento dos dados feito por partido, o que obtemos?
Para manter o padrão do conjunto de dados, o engajamento total de um partido será o somatório de todos os engajamentos totais de seus parlamentares, o engajamento mediano de um partido será a mediana de todos os engajamentos medianos de seus parlamentares e o engajamento máximo de um partido será o máximo de todos os engajamentos máximos de seus parlamentares.

Modificando então o dataset para obtermos as métricas acima citadas e filtrando novamente:
```{r}
dados_partidos = dados_parlamentares %>% group_by(partido) %>% summarise(tweets = sum(n_total), engaj_total_partido = sum(engaj_total), engaj_mediano_partido = median(engaj_mediano), engaj_max_partido = max(engaj_max))

dados_partidos_filtrado = subset(dados_partidos, !is.na(partido) & !is.na(tweets) & tweets > 0)
```

Analisando a relação entre o engajamento total dos partidos e o número de tweets:
```{r}
grafico_engaj_total_partido_x_tweets <- dados_partidos_filtrado %>%
  plot_ly(
    x = ~tweets,
    y = ~engaj_total_partido
  )

grafico_engaj_total_partido_x_tweets
```

Analisando a relação entre o engajamento mediano dos partidos e o número de tweets:
```{r}
grafico_engaj_mediano_partido_x_tweets <- dados_partidos_filtrado %>%
  plot_ly(
    x = ~tweets,
    y = ~engaj_mediano_partido
  )

grafico_engaj_mediano_partido_x_tweets
```

Analisando a relação entre o engajamento máximo dos partidos e o número de tweets:
```{r}
grafico_engaj_max_partido_x_tweets <- dados_partidos_filtrado %>%
  plot_ly(
    x = ~tweets,
    y = ~engaj_max_partido
  )

grafico_engaj_max_partido_x_tweets
```

### 2.1.

### 2.2.
